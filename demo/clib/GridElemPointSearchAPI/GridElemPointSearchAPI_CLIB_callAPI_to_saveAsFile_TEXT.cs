...
{
    /*
     * 客户端调取，格点要素抽取，存储text文件对象
     */
    class GridElemPointSearchAPI_CLIB_callAPI_to_saveAsFile_TEXT
    {
        /* 1. 定义client对象 */
        ...
        static void Main1(string[] args) {
            ...
            new GridElemPointSearchAPI_CLIB_callAPI_to_saveAsFile_TEXT().test();
        }
       public void test(){
		...
		/* 2.5 文件的本地全路径 */
		string savePath = "e:/temp/GridElemPointSearchAPI_CLIB_callAPI_to_saveAsFile_TEXT.txt";
		/* 2.6 返回文件的描述对象 */
		...
		try {
			...
			if (rst == 0) { // 正常返回
				...
				Console.WriteLine("[error] GridElemPointSearchAPI_CLIB_callAPI_to_saveAsFile_TEXT.");
