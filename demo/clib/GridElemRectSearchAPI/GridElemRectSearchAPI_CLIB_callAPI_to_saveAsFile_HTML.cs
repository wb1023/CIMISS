...
namespace cma.cimiss.demo.clib.GridElemPointSearchAPI
{
     ...
     * 格点场要素获取（切块），写HTML文件
     */
    class GridElemRectSearchAPI_CLIB_callAPI_to_saveAsFile_HTML
    {
        /* 1. 定义client对象 */
        ...
        static void Main1(string[] args) {
            ...
            new GridElemRectSearchAPI_CLIB_callAPI_to_saveAsFile_HTML().test();
        }
        public void  test(){
		...
		/* 2.5 文件的本地全路径 */
		string savePath = "e:/temp/GridElemRectSearchAPI_CLIB_callAPI_to_saveAsFile_HTML.html";
		/* 2.6 返回文件的描述对象 */
		...
		try {
			...
			if (rst == 0) { // 正常返回
				...
				Console.WriteLine("[error] GridElemRectSearchAPI_CLIB_callAPI_to_saveAsFile_HTML.");
