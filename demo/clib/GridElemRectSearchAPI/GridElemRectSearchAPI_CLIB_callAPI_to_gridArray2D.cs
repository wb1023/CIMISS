...
namespace cma.cimiss.demo.clib.GridElemRectSearchAPI
{
     ...
     */
    class GridElemRectSearchAPI_CLIB_callAPI_to_gridArray2D
    {
        /* 1. 定义client对象 */
        ...
        static void Main1(string[] args) {
            ...
            new GridElemRectSearchAPI_CLIB_callAPI_to_gridArray2D().test();
        }
        public void test(){		
		/* 1. 定义client对象 */
		...
		/* 2.4 返回对象 */
		RetGridArray2D retGridArray2D = new RetGridArray2D();
		...
		try {
			...
			int rst = client.callAPI_to_gridArray2D(userId, pwd, interfaceId,
					param, retGridArray2D);
			// 输出结果
			if (rst == 0) { // 正常返回
				ClibUtil clibUtil = new ClibUtil();
				clibUtil.outputRst(retGridArray2D);
			} else { // 异常返回
				Console.WriteLine("[error] GridElemRectSearchAPI_CLIB_callAPI_to_gridArray2D.");
