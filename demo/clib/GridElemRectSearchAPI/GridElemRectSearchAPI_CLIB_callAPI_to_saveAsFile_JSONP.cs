...
namespace cma.cimiss.demo.clib.GridElemRectSearchAPI
{
     ...
     * 格点场要素获取（切块），写JSON文件
     */
    class GridElemRectSearchAPI_CLIB_callAPI_to_saveAsFile_JSONP
    {
        /* 1. 定义client对象 */
        ...
        static void Main1(string[] args) {
            ...
            new GridElemRectSearchAPI_CLIB_callAPI_to_saveAsFile_JSONP().test();
        }
        public void test(){
		/* 1. 定义client对象 */
		...
		/* 2.5 文件的本地全路径 */
		string savePath = "e:/temp/GridElemRectSearchAPI_CLIB_callAPI_to_saveAsFile_JSONP.json";
		/* 2.6 返回文件的描述对象 */
		...
		try {
			...
			if (rst == 0) { // 正常返回
				...
				Console.WriteLine("[error] GridElemRectSearchAPI_CLIB_callAPI_to_saveAsFile_JSON.");
