...
namespace cma.cimiss.demo.clib.GridElemRectSearchAPI
{
     ...
     * 格点场要素获取（切块），写TEXT文件
     */
    class GridElemRectSearchAPI_CLIB_callAPI_to_saveAsFile_TEXT
    {
       ...
       static void Main1(string[] args) {
           ...
           new GridElemRectSearchAPI_CLIB_callAPI_to_saveAsFile_TEXT().test();
       }
       public void test()
       {
		/* 1. 定义client对象 */
		...
		/* 2.5 文件的本地全路径 */
		string savePath = "e:/temp/GridElemRectSearchAPI_CLIB_callAPI_to_saveAsFile_TEXT.txt";
		/* 2.6 返回文件的描述对象 */
		...
		try {
			...
			if (rst == 0) { // 正常返回
				...
				Console.WriteLine("[error] GridElemRectSearchAPI_CLIB_callAPI_to_saveAsFile_TEXT.");
