...
{
    /*
     * 客户端调取，站点要素统计，序列化为kml格式字符串
     * 运行错误 Element dont't exist
     */
    class StaElemStatAPI_CLIB_callAPI_to_serializedStr_KML
    {
        /* 1. 定义client对象 */
        ...
        static void Main1(string[] args) {
            ...
            new StaElemStatAPI_CLIB_callAPI_to_serializedStr_KML().test();
        }
        public void test()
        {
		/* 1. 定义client对象 */
		...
		// 必选参数
		//param.Add("elements", "Station_ID_C,staName"); //统计分组：站号，站名
		param.Add("elements", "lat,lon,SUM_PRE_1h");//纬度、经度、1小时累计降水量
	    ...
	    try {
	      ...
	      if(rst == 0) { //正常返回
	        ...
	        formatUtil.outputRstText( Convert.ToString(retStr) ) ;
	      } else { //异常返回
	        Console.WriteLine( "[error] StaElemStatAPI_CLIB_callAPI_to_serializedStr_KML.kml" ) ;       
